...
create table QRCB_UPMS.SYS_DATA_DATE
(
    ID BIGINT not null generated as identity constraint SYS_DATA_DATE_PK primary key ,
    SCHEMA VARCHAR(128) not null,
    TABLE_NAME VARCHAR(128) not null,
    REAL_TIME CHARACTER(1) default '0',
...
comment on table QRCB_UPMS.SYS_DATA_DATE is '系统最新数据日期';

comment on column QRCB_UPMS.SYS_DATA_DATE.ID is '主键';

comment on column QRCB_UPMS.SYS_DATA_DATE.SCHEMA is '数据库模式';

comment on column QRCB_UPMS.SYS_DATA_DATE.TABLE_NAME is '表名';

comment on column QRCB_UPMS.SYS_DATA_DATE.REAL_TIME is '是否为实时数据:0=否,1=是';

comment on column QRCB_UPMS.SYS_DATA_DATE.COL_NAME is '字段名';

comment on column QRCB_UPMS.SYS_DATA_DATE.DATA_DATE is '数据日期';

comment on column QRCB_UPMS.SYS_DATA_DATE.DATA_DATE_TIME is '实时数据时间';

comment on column QRCB_UPMS.SYS_DATA_DATE.CREATE_USER is '创建人';

comment on column QRCB_UPMS.SYS_DATA_DATE.CREATE_TIME is '创建时间';

comment on column QRCB_UPMS.SYS_DATA_DATE.UPDATE_USER is '更新人';

comment on column QRCB_UPMS.SYS_DATA_DATE.UPDATE_TIME is '更新时间';

comment on column QRCB_UPMS.SYS_DATA_DATE.DEL_FLAG is '删除标记:1=已删除,0=正常';

create index SYS_DATA_DATE_IDX_SCHEMA
    on QRCB_UPMS.SYS_DATA_DATE(SCHEMA);

create index SYS_DATA_DATE_IDX_TABLE_NAME
    on QRCB_UPMS.SYS_DATA_DATE(TABLE_NAME);

create index SYS_DATA_DATE_IDX_DEL_FLAG
    on QRCB_UPMS.SYS_DATA_DATE(DEL_FLAG);
...
comment on column QRCB_UPMS.SYS_USER.REAL_NAME is '姓名';
