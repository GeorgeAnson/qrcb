...
import com.qrcb.common.core.security.exception.*;
 ...
 * @Author Anson
 * @Create 2023-10-23
 * @Description OAuth Server 异常处理，重写 oauth 默认实现 <br/>
 */

@Slf4j
public class QrcbWebResponseExceptionTranslator implements WebResponseExceptionTranslator {
    ...
    public ResponseEntity<OAuth2Exception> translate(Exception e) {
        ...
        ase = (InvalidGrantException) throwableAnalyzer.getFirstThrowableOfType(InvalidGrantException.class,
                causeChain);
        if (ase != null) {
            String msg = QrcbSecurityMessageSourceUtil.getAccessor().getMessage(
    ...
    private ResponseEntity<OAuth2Exception> handleOAuth2Exception(OAuth2Exception e) {
        ...
        return new ResponseEntity<>(new QrcbAuth2Exception(e.getMessage(), e.getOAuth2ErrorCode()), headers,
